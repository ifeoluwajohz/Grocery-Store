import React, { useState } from 'react';
import { Link } from 'react-router-dom';
import { useAuth } from "../context/AuthContext"
import Category from './Category';

const Navbar: React.FC = () => {
  const { user, signOut } = useAuth()
  const [category, setCategory] = useState<boolean>(false);
  const [isLoggedIn, setIsLoggedIn] = useState<boolean>(false);
  const [cart, setCart] = useState<boolean>(false)

  const toggleCategory = () => {
    setCategory((prev) => !prev);
  };

  const handleClick = () => {
    signOut()
  }
  return (
    <>
    <div className="flex items-center justify-between md:space-x-10 space-x-3 py-2 relative">
      <div className="categories flex">
        <p
          className="text-sm cursor-pointer"
          onClick={toggleCategory}
        >
          All Categories
        </p>
      </div>

      <div className="search flex-grow relative w-0 md:w-10 invisible md:visible">
        <input
          className="text-xs font-light px-4 py-1 outline-gray-200 border border-gray-100 w-full pr-10"
          type="text"
          name="search"
          id="search"
          placeholder="Search for product, section, or category"
        />
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="absolute right-3 top-1/2 transform -translate-y-1/2 w-3 h-3 text-gray-500"
          x="0px"
          y="0px"
          viewBox="0 0 26 26"
        >
          <path d="M 10 0.1875 C 4.578125 0.1875 0.1875 4.578125 0.1875 10 C 0.1875 15.421875 4.578125 19.8125 10 19.8125 C 12.289063 19.8125 14.394531 19.003906 16.0625 17.6875 L 16.9375 18.5625 C 16.570313 19.253906 16.699219 20.136719 17.28125 20.71875 L 21.875 25.34375 C 22.589844 26.058594 23.753906 26.058594 24.46875 25.34375 L 25.34375 24.46875 C 26.058594 23.753906 26.058594 22.589844 25.34375 21.875 L 20.71875 17.28125 C 20.132813 16.695313 19.253906 16.59375 18.5625 16.96875 L 17.6875 16.09375 C 19.011719 14.421875 19.8125 12.300781 19.8125 10 C 19.8125 4.578125 15.421875 0.1875 10 0.1875 Z M 10 2 C 14.417969 2 18 5.582031 18 10 C 18 14.417969 14.417969 18 10 18 C 5.582031 18 2 14.417969 2 10 C 2 5.582031 5.582031 2 10 2 Z"></path>
        </svg>
      </div>

      <div className="notifications flex md:space-x-6 space-x-3 items-center">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="18"
          height="18"
          viewBox="0 0 50 50"
        >
          <path d="M 25 0 C 22.800781 0 21 1.800781 21 4 C 21 4.515625 21.101563 5.015625 21.28125 5.46875 C 15.65625 6.929688 12 11.816406 12 18 C 12 25.832031 10.078125 29.398438 8.25 31.40625 C 7.335938 32.410156 6.433594 33.019531 5.65625 33.59375 C 5.265625 33.878906 4.910156 34.164063 4.59375 34.53125 C 4.277344 34.898438 4 35.421875 4 36 C 4 37.375 4.84375 38.542969 6.03125 39.3125 C 7.21875 40.082031 8.777344 40.578125 10.65625 40.96875 C 13.09375 41.472656 16.101563 41.738281 19.40625 41.875 C 19.15625 42.539063 19 43.253906 19 44 C 19 47.300781 21.699219 50 25 50 C 28.300781 50 31 47.300781 31 44 C 31 43.25 30.847656 42.535156 30.59375 41.875 C 33.898438 41.738281 36.90625 41.472656 39.34375 40.96875 C 41.222656 40.578125 42.78125 40.082031 43.96875 39.3125 C 45.15625 38.542969 46 37.375 46 36 C 46 35.421875 45.722656 34.898438 45.40625 34.53125 C 45.089844 34.164063 44.734375 33.878906 44.34375 33.59375 C 43.566406 33.019531 42.664063 32.410156 41.75 31.40625 C 39.921875 29.398438 38 25.832031 38 18 C 38 11.820313 34.335938 6.9375 28.71875 5.46875 C 28.898438 5.015625 29 4.515625 29 4 C 29 1.800781 27.199219 0 25 0 Z M 25 2 C 26.117188 2 27 2.882813 27 4 C 27 5.117188 26.117188 6 25 6 C 23.882813 6 23 5.117188 23 4 C 23 2.882813 23.882813 2 25 2 Z M 27.34375 7.1875 C 32.675781 8.136719 36 12.257813 36 18 C 36 26.167969 38.078125 30.363281 40.25 32.75 C 41.335938 33.941406 42.433594 34.6875 43.15625 35.21875 C 43.515625 35.484375 43.785156 35.707031 43.90625 35.84375 C 44.027344 35.980469 44 35.96875 44 36 C 44 36.625 43.710938 37.082031 42.875 37.625 C 42.039063 38.167969 40.679688 38.671875 38.9375 39.03125 C 35.453125 39.753906 30.492188 40 25 40 C 19.507813 40 14.546875 39.753906 11.0625 39.03125 C 9.320313 38.671875 7.960938 38.167969 7.125 37.625 C 6.289063 37.082031 6 36.625 6 36 C 6 35.96875 5.972656 35.980469 6.09375 35.84375 C 6.214844 35.707031 6.484375 35.484375 6.84375 35.21875 C 7.566406 34.6875 8.664063 33.941406 9.75 32.75 C 11.921875 30.363281 14 26.167969 14 18 C 14 12.261719 17.328125 8.171875 22.65625 7.21875 C 23.320313 7.707031 24.121094 8 25 8 C 25.886719 8 26.679688 7.683594 27.34375 7.1875 Z M 21.5625 41.9375 C 22.683594 41.960938 23.824219 42 25 42 C 26.175781 42 27.316406 41.960938 28.4375 41.9375 C 28.792969 42.539063 29 43.25 29 44 C 29 46.222656 27.222656 48 25 48 C 22.777344 48 21 46.222656 21 44 C 21 43.242188 21.199219 42.539063 21.5625 41.9375 Z"></path>
        </svg>
        <img 
          className="w-5"
          onClick={() =>setCart(!cart)}
          src="https://img.icons8.com/ios/50/shopping-cart-loaded--v1.png"
          alt=""
        />
        <img onClick={() =>setIsLoggedIn(!isLoggedIn)} className="w-6 h-6" src="https://img.icons8.com/fluency/48/test-account--v1.png" alt="test-account" />
        {cart && (
          <div className="text-right mt-2 loggedin fixed  top-8 right-2 lg:right-32 z-50 bg-slate-200 p-4 w-64 h-screen space-y-4">
            {user ? (
              <>
                <Link to="/Settings" className="block text-blue-600 hover:text-blue-800 font-medium">
                  Account Info
                </Link>
                <Link to="/Account_management" className="block text-blue-600 hover:text-blue-800 font-medium">
                  Sign Out
                </Link>
                <p onClick={handleClick}className="block text-red-600 hover:text-red-800 font-medium cursor-pointer">Logout</p>
              </>
            ) : (
              <div className="space-y-4">
                <Link to="/Login" className="block text-blue-600 text-xs hover:text-blue-800 mt-6" > Login </Link>
                <Link to="/Login" className="block text-blue-600 text-xs hover:text-blue-800" > SAy HI </Link>
              </div>
            )}
          </div>
        )}
        {isLoggedIn && (
          <div className="text-right mt-2 loggedin fixed  top-8 right-2 lg:right-32 z-50 bg-slate-200 p-4 w-64 space-y-4">
            {user ? (
              <>
                <Link to="/Settings" className="block text-blue-600 hover:text-blue-800 font-medium">
                  Account Info
                </Link>
                <Link to="/Account_management" className="block text-blue-600 hover:text-blue-800 font-medium">
                  Sign Out
                </Link>
                <p onClick={handleClick}className="block text-red-600 hover:text-red-800 font-medium cursor-pointer">Logout</p>
              </>
            ) : (
              <div className="space-y-4">
                <Link to="/Login" className="block text-blue-600 text-xs hover:text-blue-800 mt-6" > Login </Link>
                <Link to="/Login" className="block text-blue-600 text-xs hover:text-blue-800" > Fuck You </Link>
                <Link to="/Login" className="block text-blue-600 text-xs hover:text-blue-800" > SAy HI </Link>
                <Link to="/Login" className="block text-blue-600 text-xs hover:text-blue-800" > Fuck You </Link>
                <Link to="/Login" className="block text-blue-600 text-xs hover:text-blue-800" > SAy HI </Link>

              </div>
            )}
          </div>
        )}

      </div>

    </div>

    <div className="search flex-grow relative md:w-0 w-full md:invisible visible mb-10">
        <input
          className="text-xs font-light px-4 py-1 outline-gray-300 border border-gray-200 w-full pr-10"
          type="text"
          name="search"
          id="search"
          placeholder="Search for product, section, or category"
        />
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="absolute right-3 top-1/2 transform -translate-y-1/2 w-3 h-3 text-gray-500"
          x="0px"
          y="0px"
          viewBox="0 0 26 26"
        >
          <path d="M 10 0.1875 C 4.578125 0.1875 0.1875 4.578125 0.1875 10 C 0.1875 15.421875 4.578125 19.8125 10 19.8125 C 12.289063 19.8125 14.394531 19.003906 16.0625 17.6875 L 16.9375 18.5625 C 16.570313 19.253906 16.699219 20.136719 17.28125 20.71875 L 21.875 25.34375 C 22.589844 26.058594 23.753906 26.058594 24.46875 25.34375 L 25.34375 24.46875 C 26.058594 23.753906 26.058594 22.589844 25.34375 21.875 L 20.71875 17.28125 C 20.132813 16.695313 19.253906 16.59375 18.5625 16.96875 L 17.6875 16.09375 C 19.011719 14.421875 19.8125 12.300781 19.8125 10 C 19.8125 4.578125 15.421875 0.1875 10 0.1875 Z M 10 2 C 14.417969 2 18 5.582031 18 10 C 18 14.417969 14.417969 18 10 18 C 5.582031 18 2 14.417969 2 10 C 2 5.582031 5.582031 2 10 2 Z"></path>
        </svg>
    </div>
    {category && (
      <div className="absolute top-10 w-full h-1/3 bg-slate-100 p-4 shadow-md">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-bold">Categories</h2>
          <button onClick={toggleCategory} className="text-gray-600 hover:text-gray-900">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 w-6"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M6 18L18 6M6 6l12 12"
              />
            </svg>
          </button>
        </div>
        <Category />
      </div>
    )}
    </>
  );
};

export default Navbar;
